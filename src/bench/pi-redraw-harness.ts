...
import {
  ...
  type DrainScrollbackOverflowResult,
} from "../effect/services/pty/scrollback-archiver"
...
export function buildSyntheticPiWorkload(options?: {
  entries?: number
}): string {
...
export async function runPiRedrawBenchmark(
  options: RunPiRedrawBenchmarkOptions = {}
): Promise<PiRedrawBenchmarkResult | PiRedrawBenchmarkError> {
  const cols = options.cols ?? DEFAULT_COLS
  const rows = options.rows ?? DEFAULT_ROWS
  const repaints = options.repaints ?? DEFAULT_REPAINTS
  const iterations = options.iterations ?? DEFAULT_ITERATIONS
  const warmupIterations = options.warmupIterations ?? DEFAULT_WARMUP_ITERATIONS
  const archiveWorkloadRowLimit =
    options.archiveWorkloadRowLimit ?? DEFAULT_ARCHIVE_WORKLOAD_ROW_LIMIT

  const workloadResult = resolveWorkload({
    archiveMetaPath: options.archiveMetaPath,
    archiveWorkloadRowLimit,
    archiveWindowStart: options.archiveWindowStart,
    archiveWindowRows: options.archiveWindowRows,
    syntheticEntries: options.syntheticEntries ?? DEFAULT_SYNTHETIC_ENTRIES,
    workloadText: options.workloadText,
  })
  if (workloadResult instanceof PiRedrawBenchmarkError) {
    return workloadResult
  }

  const { workloadText, workloadKind, workloadDescription, archiveAnalysis } = workloadResult
  const warmupRuns = Math.max(0, warmupIterations)
  const measuredRuns = Math.max(1, iterations)

  for (let index = 0; index < warmupRuns; index++) {
    const warmup = await runSingleScenarioPair({
      cols,
      rows,
      repaints,
      workloadText,
    })
    if (warmup instanceof PiRedrawBenchmarkError) {
      return warmup
    }
  }

  const samples: Array<{ append: BenchmarkScenarioMetrics; repaint: BenchmarkScenarioMetrics }> = []
  for (let index = 0; index < measuredRuns; index++) {
    const sample = await runSingleScenarioPair({
      cols,
      rows,
      repaints,
      workloadText,
    })
    if (sample instanceof PiRedrawBenchmarkError) {
      return sample
    }
    samples.push(sample)
  }

  const append = aggregateScenarioMetrics(samples.map((sample) => sample.append))
  const repaint = aggregateScenarioMetrics(samples.map((sample) => sample.repaint))
  ...
  return {
    ...
    workload: {
      ...
      bytes: Buffer.byteLength(workloadText, "utf8"),
      lines: countLines(workloadText),
    },
    archiveAnalysis,
    append,
    repaint,
    repaintOverAppend: append.totalMs === 0 ? 0 : repaint.totalMs / append.totalMs,
  }
}
...
function collectArchiveRows(options: {
  ...
  for (const chunkInfo of options.meta.chunks) {
    ...
    for (const row of chunkRows) {
      ...
      if (
        workloadRows.length < options.workloadRowLimit &&
        !seenWorkloadRows.has(text)
      ) {
...
function primeGhosttyLibraryEnv(): void {
  ...
  const candidates = [
    ...
    path.join(process.cwd(), "native", "zig-ghostty-wrapper", "zig-out", "lib", `libghostty-vt.${ext}`),
...
function aggregateScenarioMetrics(
  samples: BenchmarkScenarioMetrics[]
