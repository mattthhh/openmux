...
export const WorkspaceIdSchema = z.number().int().min(1).max(9).transform((n) => n as WorkspaceId)
...
export const ColsSchema = z.number().int().positive().transform((n) => n as Cols)

/** Terminal row count (must be positive) */
export type Rows = Brand<number, "Rows">
export const RowsSchema = z.number().int().positive().transform((n) => n as Rows)
