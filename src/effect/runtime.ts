...
/**
 * Run an Effect - stub that just returns an error as Effect is no longer used
 * @deprecated Use errore services directly
 */
export function runEffect<T>(_effect: unknown): Promise<T | Error> {
  console.warn('Effect runtime is deprecated, use errore services directly');
  return Promise.resolve(new Error('Effect runtime no longer available. Use errore services.'));
}
