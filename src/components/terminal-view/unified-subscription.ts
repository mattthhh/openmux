          ...
          const unsubResult = await errore.tryAsync<() => void, TerminalSubscriptionError>({
            try: () =>
              subscribeUnifiedToPty(ptyId, (update: UnifiedTerminalUpdate) => {
                ...
                // In shim/client mode, the server sends a stale viewportOffset
                // (always 0) because the client's animator never syncs scroll
                // position to the server. Reject viewportOffset=0 from server
                // updates when the client already has a non-zero offset.
                const animating = terminal.isAnimating(ptyId);
                const existingScroll = viewState.scrollState;
                const shimRejectZero =
                  isShimClient() && update.scrollState.viewportOffset === 0 && !animating;
                if (existingScroll) {
                  if (!animating) {
