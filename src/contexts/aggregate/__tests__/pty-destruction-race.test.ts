...
import { createAggregateViewRefreshers, createLifecycleHandlers } from '../../aggregate-view-subscriptions';
...
import { listAllPtysWithMetadata, getAggregateSessionPtyMapping } from '../../../effect/bridge/aggregate-bridge';
import { listSessionsResult, getSessionSummaryResult, loadSession } from '../../../effect/bridge/session-bridge';
...
describe('PTY destruction race condition', () => {
  ...
  const createTestHarness = () => {
    ...
    const getCurrentSessionPaneOrder = () => new Map([['pane-1', 0], ['pane-2', 1]]);
    ...
    return { state, setState, refreshPtys, lifecycleHandlers, refreshState };
  };
