...
describe('scroll state: real code integration', () => {
  ...
  test('guard: subscriber viewportOffset=0 rejected unless user requested scrollToBottom', async () => {
    // User scrolls up
    scrollHandlers.scrollTerminal(PTY_ID, 50);
    await drain();
    expect(viewOffset).toBe(50);
    ...
    // Subscriber: isAnimating=false, updateOffset=0,
    // viewState.viewportOffset=50 > 2, wasScrollToBottomRequested=false
    // → SHOULD REJECT the zero
    const animating = scrollHandlers.isAnimating(PTY_ID);
    ...
    if (!animating) {
      if (
        rogueUpdate.viewportOffset === 0 &&
        viewOffset > 2 &&
        !scrollHandlers.wasScrollToBottomRequested(PTY_ID)
      ) {
        // Skip — guard rejects the stale zero
      } else {
        viewOffset = rogueUpdate.viewportOffset;
      }
    }
