...
describe('Domain Models', () => {
  describe('Rectangle', () => {
    ...
    it('rejects invalid dimensions via schema validation', () => {
      ...
      expect(() => RectangleSchema.parse({ x: 0, y: 0, width: 0, height: 50 })).toThrow();
      expect(() => RectangleSchema.parse({ x: 0, y: 0, width: 100, height: -1 })).toThrow();
    });
  });
...
describe('Schema Encoding/Decoding', () => {
  describe('SerializedSession', () => {
    ...
    it('rejects invalid session JSON', () => {
      ...
      expect(() => SerializedSessionSchema.parse(json)).toThrow();
    });
  });
});
