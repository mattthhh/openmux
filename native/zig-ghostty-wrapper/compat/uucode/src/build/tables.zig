...
pub fn main() !void {
    ...
    const resolved_tables = comptime blk: {
        var tmp: [table_configs.len]config.Table = undefined;
        for (table_configs, 0..) |table_config, i| {
            tmp[i] = table_config.resolve();
        }
        break :blk tmp;
    };
...
fn TableAllData(comptime c: config.Table) type {
    ...
    for (c.extensions) |x| {
        loop_inputs: for (x.inputs) |input| {
            for (field_names[0..i]) |existing| {
                if (std.mem.eql(u8, existing, input)) {
    ...
    return @Struct(.auto, null, all_field_names, field_types[0..i], field_attrs[0..i]);
}
