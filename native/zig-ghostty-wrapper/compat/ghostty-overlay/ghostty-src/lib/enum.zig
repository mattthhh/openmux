...
pub fn Enum(
    ...
    const field_count = comptime count: {
        var count_: usize = 0;
        for (keys) |key_| {
            if (key_ != null) count_ += 1;
        }
        break :count count_;
    };
    ...
    const Tag = switch (target) {
        .c => c_int,
        .zig => std.math.IntFittingRange(0, if (field_count == 0) 0 else field_count - 1),
    };

    var names: [field_count][]const u8 = undefined;
    var values: [field_count]Tag = undefined;
    ...
    for (keys) |key_| {
        ...
        names[fields_i] = key;
        values[fields_i] = @intCast(fields_i + holes);
        fields_i += 1;
    }
