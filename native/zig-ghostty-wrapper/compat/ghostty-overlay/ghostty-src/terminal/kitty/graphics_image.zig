...
const ghostty_io = @import("../../ghostty_io.zig");
...
pub const LoadingImage = struct {
    ...
    pub fn init(
        ...
        // Verify our capabilities and limits allow this.
        {
            ...
            switch (t.medium) {
                ...
                .shared_memory => return error.UnsupportedMedium,
            }
        }
        ...
        var abs_buf: [std.fs.max_path_bytes:0]u8 = undefined;
        const path = switch (t.medium) {
            .direct => unreachable, // handled above
            .file, .temporary_file => path: {
                const path_z = try alloc.dupeZ(u8, cmd.data);
                defer alloc.free(path_z);
                const resolved = std.c.realpath(path_z, &abs_buf) orelse {
                    log.warn("failed to get absolute path: realpath", .{});
                    return error.InvalidData;
                };
                break :path std.mem.sliceTo(resolved, 0);
            },
    ...
    fn readFile(
        ...
        defer if (medium == .temporary_file) {
            ...
            if (std.c.unlink(path_z) != 0) {
                log.warn("failed to delete temporary file", .{});
            }
        };

        const io = ghostty_io.get();
    ...
    fn isPathInTempDir(path: []const u8) bool {
        ...
        if (std.mem.startsWith(u8, path, "/private/var")) return true;
        if (std.mem.startsWith(u8, path, "/dev/shm")) return true;
        if (std.c.getenv("TMPDIR")) |tmpdir_z| {
            const tmpdir = std.mem.sliceTo(tmpdir_z, 0);
            if (std.mem.startsWith(u8, path, tmpdir)) return true;
        }
        return false;
    }
